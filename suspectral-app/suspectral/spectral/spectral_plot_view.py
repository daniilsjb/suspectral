...
class SpectralPlotView(pg.PlotWidget):
    def __init__(self, exporters: list[Exporter], parent: QWidget | None = None):
    ...
    def set_wavelengths(self, wavelengths: np.ndarray, unit: str | None = None):
        ...
        self.setLabel("bottom", f"Wavelength ({unit})" if unit else "Wavelength")
    ...
    def _export_spectra(self, exporter: Exporter):
        exporter.export(np.array(self._spectra), self._wavelengths)
