...
class StatusBar(QStatusBar):
    ...
    def update_hypercube_status(self, hypercube: Hypercube):
        ...
        self._wavelength_status.set(
            ...
            wavelengths_unit=hypercube.wavelengths_unit,
        )
        self._memory_status.set(
            num_bytes=hypercube.num_bytes,
        )

    def update_cursor_status(self, position: QPoint | None = None):
        self._cursor_status.set(position)

    def update_selection_status(self, rectangle: QRect | None = None):
...
class CursorStatus(StatusBarItem):
    ...
    def set(self, position: QPoint | None = None):
...
class SelectionStatus(StatusBarItem):
    ...
    def set(self, rectangle: QRect | None = None):
...
class WavelengthStatus(StatusBarItem):
    ...
    def set(self, wavelengths: np.ndarray | None = None, wavelengths_unit: str | None = None):
        ...
        unit = f" ({wavelengths_unit})" if wavelengths_unit is not None else ""
        diff = np.round(np.diff(wavelengths), 2)
        step = f" : {diff[0]:g}" if np.all(diff == diff[0]) else ""

        wmin = np.round(wavelengths.min(), 2)
        wmax = np.round(wavelengths.max(), 2)

        self._label.setText(f"{wmin:g} : {wmax:g}{step}{unit}")
