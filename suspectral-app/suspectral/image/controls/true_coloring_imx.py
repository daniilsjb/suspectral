...
class TrueColoringIMX(ColoringMode):
    def __init__(self, container: HypercubeContainer, parent: QWidget | None = None):
        ...
        layout.addWidget(QLabel("There are no configurable options for this mode."))
    ...
    def _handle_synthesis(self):
        ...
        self._worker = SynthesizeIMXWorker(self._container.hypercube)
        ...
        self._worker.produced.connect(self.image_changed.emit)
...
class SynthesizeIMXWorker(QObject):
    progress = Signal(int)
    finished = Signal()
